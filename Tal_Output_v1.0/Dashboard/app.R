...
ui <- dashboardPage(
  ...
  # HEADER
  dashboardHeader(title = "StarCruiser Economic Dashboard"),

  # SIDEBAR
  dashboardSidebar(
    sidebarMenu(
      menuItem("Overview", tabName = "overview", icon = icon("dashboard")),
      menuItem("CPI/PCE Inflation", tabName = "inflation", icon = icon("chart-line")),
      menuItem("Yield Curve", tabName = "yields", icon = icon("chart-area")),
      menuItem("Employment", tabName = "employment", icon = icon("users")),
      menuItem("Labor Dynamics", tabName = "labor_dynamics", icon = icon("exchange-alt")),
      menuItem("Wages & Hours", tabName = "wages_hours", icon = icon("money-bill-wave")),
      menuItem("Geographic", tabName = "geographic", icon = icon("map")),
      menuItem("Treasury Market", tabName = "treasury", icon = icon("university")),
      menuItem("Inflation Deep Dive", tabName = "inflation_deep", icon = icon("fire")),
  ...
  dashboardBody(
    ...
    tags$footer(
      ...
      HTML("
        ...
        <strong>Dashboard</strong>: StarCruiser v2.0 |
...
server <- function(input, output, session) {
  ...
  output$plot_yield_snapshot <- renderEcharts4r({
    ...
    yields <- rates_data[date == snapshot_date &
      series_id %chin% c("DGS3MO", "DGS2", "DGS5", "DGS10", "DGS30")]
